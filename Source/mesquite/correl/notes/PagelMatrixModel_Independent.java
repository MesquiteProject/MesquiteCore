...
public class PagelMatrixModel extends MultipleProbCategCharModel implements Evaluator,CLikelihoodCalculator {
	...
	public static final int INDEP4PARAM = 9;   //Model with two completely separate characters (e.g., no zero diagonal)
	...
	double [] savedRootEstimates; // simulation prior for univariate models
	double[][] savedRootEstimates2; // holds the estimates at the root (for simulation priors, etc.)
	...
	AsymmModel m2p1 = null;
	AsymmModel m2p2 = null;
	
	// stuff for single element evaluation
	double[][] finalProbs = null;
	double[] underflowCompUp = null;
	double []empirical = null;
	...
	public double evaluate(double[] values, Object obj) {
		if (values.length >= 4) {
			...
			return result;
				...
				TreeDataModelBundle b1 = new TreeDataModelBundle(workingTree, null, observedStates1, params, this, null);
				double[] m1params = new double[2];
				m1params[0] = m2p1.getParam0();
				m1params[1] = m2p1.getParam1();
				double lk1 = m2p1.evaluate(m1params,b1);
				TreeDataModelBundle b2 = new TreeDataModelBundle(workingTree,null,observedStates2,params,this,null);
				double[] m2params = new double[2];
				m2params[0] = m2p2.getParam0();
				m2params[1] = m2p2.getParam1();				
				double lk2 = m2p2.evaluate(m2params,b2);
				return (lk1+lk2);
			}
		}
		else
	...
	public double logLikelihoodCalc(Tree tree){
		...
		if (rootMode == SUM){
			...
			for (int i = 0; i<=maxState[0]; i++)
				for (int j = 0; j<=maxState[1];j++) {
					savedRootEstimates2[j][i] = downProbs2[root][j][i];
					if (savedRootEstimates2[j][i] < 0){      // problem, probably due to bad matrix operation
						likelihood = 0;
						return(-1*MesquiteDouble.veryLargeNumber);
					}
					else
						likelihood += savedRootEstimates2[j][i];
				}
		}
		//else 
 	...
 	private void initProbs2(int nodes, int numStates1, int numStates2) {
 		...
 		if  (downProbs2==null || downProbs2.length!=nodes || downProbs2[0].length!=numStates1 || downProbs2[0][0].length!=numStates2){
 		 		...
 		 		savedRootEstimates2 = new double[numStates][numStates];
 		}
 		...
 		Double2DArray.deassignArray(savedRootEstimates2);
 	}
	...
	public double logLikelihoodCalc(Tree tree, ProbabilityModel pModel, CharacterDistribution states){
		...
		for (int i=0;  i<=model.getMaxState(); i++) {
			...
			savedRootEstimates[i] = downProbs[root][i];
			likelihood += savedRootEstimates[i];
		}
		comp = underflowCompDown[root];
		double logLike = Math.log(likelihood) - comp;
		if (logLike> -0.00001) {
	...
	private void initProbs(int nodes, int numStates) {
		...
		if (downProbs==null || downProbs.length!=nodes || downProbs[0].length!=numStates){
			...
			empirical = new double[numStates];
		 	savedRootEstimates = new double[numStates];
		}
	...
	/* overload from bivariate version above. */
	...
	/*.................................................................................................................*/
	/* overloaded version of bivariate version above */
	private double checkUnderflow(double[] probs){
		minChecker.setValue(MesquiteDouble.unassigned);
 ...
 //   		return;
		...
		if (scaleRescale && modelType != INDEP4PARAM){  //rescales tree to 1.0 height to help optimization, which takes longer with larger numbers
		...
		switch (modelType) {
			case MODEL8PARAM:{  // first try two versions of a 6-parameter model
				...
				if (parametersFromSimplerModel == null || simplerModelType != MODEL6PARAMCONTINGENTCHANGEY){
					...
					model6_cy.estimateParameters(originalTree,observedStates1, observedStates2, commandRec);
				...
				if (parametersFromSimplerModel == null || simplerModelType != MODEL6PARAMCONTINGENTCHANGEX){
					...
					model6_cx.estimateParameters(originalTree,observedStates1, observedStates2, commandRec);
				...
				if (eightParameterExtraSearch.getValue() > 0) {
					...
					for (int i = 0; i< eightParameterExtraSearch.getValue();i++){
						for(int j=0;j<4;j++)
							estParams[j]=Math.abs((m6_cy_Params[j]+m6_cx_Params[j])/2 + (50.0*rng.nextGaussian())*(m6_cx_Params[j]-m6_cy_Params[j]));
						estParams[4]=Math.abs(1+50.0*rng.nextGaussian())*m6_cy_Params[4];
						estParams[5]=Math.abs(1+50.0*rng.nextGaussian())*m6_cx_Params[4];
						estParams[6]=Math.abs(1+50.0*rng.nextGaussian())*m6_cy_Params[5];
						estParams[7]=Math.abs(1+50.0*rng.nextGaussian())*m6_cx_Params[5];
						allowedEdgeHits = beginningAllowed;
						//backupEst = (double [])estParams.clone();
						next = opt.optimize(estParams, null); // try optimizing from the contingentchangeX best parameters
						if (!acceptableResults(next, estParams)) {
			...
			case MODEL6PARAMCONTINGENTCHANGEY: {
				...
				if (parametersFromSimplerModel == null || simplerModelType != MODEL4PARAM) {
					...
					model4.estimateParameters(originalTree,observedStates1, observedStates2, commandRec);
			...
			case INDEP4PARAM:{
				if (m2p1 == null)
					m2p1 = new AsymmModel(this.name+" Asymm helper1", CategoricalState.class);
				if (m2p2 == null)
					m2p2 = new AsymmModel(this.name+" Asymm helper2", CategoricalState.class);
				m2p1.deassignParameters();
				m2p2.deassignParameters();
				m2p2.estimateParameters(workingTree,observedStates2,this,null);
				m2p1.estimateParameters(workingTree, observedStates1, this,null);
				if (params == null || params.length != 4)
					params = new double[4];
				params[0]=m2p2.getParam0();
				params[1]=m2p1.getParam0();
				params[2]=m2p2.getParam1();
				params[3]=m2p1.getParam1();
				double best = evaluate(params,null);
				//workingTree = originalTree;

				if (commandRec != null) commandRec.tick("4 parameter independent model -ln likelihood: " + MesquiteDouble.toString(best));
				break;
			}
		...
		if (scaleRescale && height != 0 && modelType !=INDEP4PARAM){ //UNDO the scaling of the tree
    ...
    public double[][] getRootPriors(){
    		double[][] rootPriors;
    		if (savedRootEstimates2 == null)
    			return null;
    		else {
    			double total = 0;
    			int estimateLength = savedRootEstimates2.length;  // what are these really?
    			int estimateWidth = savedRootEstimates2[0].length;
    			rootPriors = new double[estimateLength][estimateWidth];
    			for (int i=0;i<estimateLength;i++)
    				for (int j=0;j<estimateWidth;j++)
    					total += savedRootEstimates2[i][j];
    			for (int i=0;i<estimateLength;i++)
    				for (int j=0;j<estimateWidth;j++)
    					rootPriors[i][j] = savedRootEstimates2[i][j]/total;
    		}
    		return rootPriors;
    }
	...
	public int[] evolveState (int beginState[], Tree tree, int node){
		if (this.modelType == PagelMatrixModel.INDEP4PARAM)
			return evolveStateFromPairedModel(beginState,tree,node);
...
//		}
	...
	public int[] evolveStateFromPairedModel(int beginState[], Tree tree, int node){
		return null;
	}
	...
	public String getModelTypeAsString() {
		switch (modelType){
		    ...
		    case INDEP4PARAM:{
			    return "4 Parameter independent model";
		    }
			case MODEL8PARAM:{
	...
	public void copyToClone(CharacterModel md){
		...
		if (model.modelType == PagelMatrixModel.INDEP4PARAM){
			...
			model.savedRootEstimates2 = (double [][])savedRootEstimates2;
