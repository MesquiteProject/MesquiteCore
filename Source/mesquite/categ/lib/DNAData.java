...
public class DNAData extends MolecularData {
	...
	public int getStartOfNextCodon(int ic) {
		while (ic<getNumChars()){  //Debugg.println: this was broken in 3.03.  I added this outer while loop.  This would only look at the first candidate; if there were two 1's in a row, it would find no more codons
			...
			int[] triplet = getCodonTriplet(candidate);  //Debugg.println: this is a costly method; an integer array created each time.  Better to pass in for re-use
			if (triplet!=null)
				return candidate;
		}
		return -1;
