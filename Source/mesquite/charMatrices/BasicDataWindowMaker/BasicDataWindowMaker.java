...
class BasicDataWindow extends TableWindow implements MesquiteListener {
	...
	public BasicDataWindow(BasicDataWindowMaker ownerModule, CharacterData data) {
		...
		MesquiteSubmenuSpec softnessSubmenu = ownerModule.addSubmenu(ownerModule.displayMenu, "Emphasis");
		//COLORS
	...
	public Object doCommand(String commandName, String arguments, CommandChecker checker) {
		...
		/**/
			...
			if (table != null && data != null) {
				DataUtility tda = null;
				if (!StringUtil.blank(arguments)){
					tda= (DataUtility)ownerModule.hireNamedEmployee(DataUtility.class, arguments);
					if (data.isEditInhibited() && !(tda instanceof DataUtilityNoAlterer)) {
						ownerModule.discreetAlert("This matrix is marked as locked against editing. To unlock, uncheck the menu item Matrix>Current Matrix>Editing Not Permitted");
						return null;
					}
				}
					...
					if (data.isEditInhibited())
						tda= (DataUtilityNoAlterer)ownerModule.hireNamedEmployee(DataUtilityNoAlterer.class, arguments);
					else
						tda= (DataUtility)ownerModule.hireNamedEmployee(DataUtility.class, arguments);
				}
		...
		else if (checker.compare(this.getClass(), "Sets whether or not terminal gaps are dark.", "[on or off]", commandName, "toggleShowDarkTerminalGaps")) {
			showDarkTerminalGaps.toggleValue(ParseUtil.getFirstToken(arguments, pos));
			...
			if (getMode() > 0)
				...
				if (parameters != null && parameters.length > 1) {
					...
					if (data.getNumChars() < table.getNumColumns()) {
						table.deleteColumns(starting,  starting + num - 1); 
					}
				}
...
class MatrixTable extends mesquite.lib.table.CMTable implements MesquiteDroppedFileHandler {
	...
	public MatrixTable(BasicDataWindowMaker editorModule, BasicDataWindow window, CharacterData data, int numRowsTotal, int numColumnsTotal, int totalWidth, int totalHeight, int taxonNamesWidth) {
		...
		showDarkTerminalGaps = new MesquiteBoolean(false);
	...
	public void setShowDarkEmptySequences(boolean showDarkEmptySequences) {
		this.showDarkEmptySequences.setValue(showDarkEmptySequences);
	}
	...
	public synchronized void drawMatrixCell(Graphics g, int x, int y, int w, int h, int column, int row, boolean selected, boolean writeStates, boolean leaveEdges) {
		...
		if (showDarkTerminalGaps.getValue()) {
			if (data.isInapplicable(column, row)) {
				boolean noDataToLeft = !data.hasDataToLeft(column, row);
				boolean noDataToRight = !data.hasDataToRight(column, row);
				if (noDataToLeft || noDataToRight)
					fillColor = Color.gray;
			}
		}
		if (showDarkEmptySequences.getValue()) {
	...
	protected void appendToClearReport(String s) {
		if (MesquiteTrunk.recordMatrixEdits) {
			if (!MesquiteFile.fileExists(MesquiteTrunk.matrixEditsReportFilePath))
				MesquiteFile.putFileContents(MesquiteTrunk.matrixEditsReportFilePath, s, true);
			else
				MesquiteFile.appendFileContents(MesquiteTrunk.matrixEditsReportFilePath, s, true);
		}
	}
	...
	protected void clearIt(boolean cut) {
		...
		for (int j = 0; j < numRowsTotal; j++) {
			...
			if (MesquiteTrunk.recordMatrixEdits) {
				if (isRowSelected(j))
					appendToClearReport(data.getName()+"\t" + taxa.getTaxonName(j)+"\t[" + StringUtil.getDateTime()+"]\tclear\n");
			}
		}
		if (cut) {
			deleteSelected();
