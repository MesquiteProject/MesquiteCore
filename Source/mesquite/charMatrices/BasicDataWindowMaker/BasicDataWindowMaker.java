...
public class BasicDataWindowMaker extends DataWindowMaker implements CommandableOwner  {
	public void getEmployeeNeeds(){  //This gets called on startup to harvest information; override this and inside, call registerEmployeeNeed
		EmployeeNeed e = registerEmployeeNeed(CellColorer.class, getName() + " uses various methods to color the cells of the character matrix.",
		"You can request the method to color cells in the Color Cells submenu of the Matrix menu.  You can also color the text of the cell using the Color Text submenu. ");
		EmployeeNeed e2 = registerEmployeeNeed(DataWindowAssistantI.class, getName() + " can employ various modules to add features to the Character Matrix Editor.",
		"The modules assisting the Character Matrix Editor are available typically in the Matrix menu of the editor window. ");
		EmployeeNeed e3 = registerEmployeeNeed(DataWindowAssistantA.class, "Various analyses can be done within the context of the Character Matrix Editor.",
		"You can request such an analysis in the Analysis menu of the Character Matrix Editor. ");
		EmployeeNeed e4 = registerEmployeeNeed(DataUtility.class, "Utilities are available to modify or summarize a matrix in the Character Matrix Editor.",
		"You can request such a utility in the Utilities submenu of the Matrix menu of the Character Matrix Editor. ");
	}
...
class BasicDataWindow extends TableWindow implements MesquiteListener {
	...
	public BasicDataWindow (BasicDataWindowMaker ownerModule, CharacterData data) {
		...
		MesquiteSubmenuSpec mssa = ownerModule.addSubmenu(MesquiteTrunk.analysisMenu, "Analyses for Matrix", MesquiteModule.makeCommand("newAssistant",  this), DataWindowAssistantA.class);
		...
		/*
		TableTool colorWandTool = new TableTool(this, "colorMagicWand", ownerModule.getPath(), "colorWand.gif", 1,1,"Select same color", "This tool selects cells of the same color", MesquiteModule.makeCommand("selectSameColor",  this) , null, null);
		colorWandTool.setWorksOnColumnNames(false);
		colorWandTool.setWorksOnRowNames(false);
		colorWandTool.setWorksOnMatrixPanel(true);
		colorWandTool.setWorksOnCornerPanel(false);
		addTool(colorWandTool);
		/*

		scrollTool = new TableTool(this, "simScroller", ownerModule.getPath(), "simScrollerRight.gif", 8, 8,"Scrolls between similar items", "This tool scrolls to other similar items", MesquiteModule.makeCommand("simScroll",  this) , null, null);
		...
		scrollTool.setOptionsCommand(MesquiteModule.makeCommand("scrollToolOptions", this));
		addTool(scrollTool);
	...
	public Object doCommand(String commandName, String arguments, CommandChecker checker) {
		...
		/*
    	 	else if (checker.compare(this.getClass(),  "Selects cells of same color", "[column touched][row touched]", commandName, "selectSameColor")) {
	   	 		if (data == null || table == null || table.cellColorer == null)
	   	 			return null;
...
class MatrixTable extends mesquite.lib.table.CMTable implements MesquiteDroppedFileHandler{
	...
	void linkTable(MesquiteTable table, boolean linkeeIsNew){
		...
		if (linkeeIsNew){
			for (int ic = 0; ic<table.getNumColumns(); ic++) //synchronizing at start
				for (int it = 0; it<table.getNumRows(); it++){
		...
		if (v != null)
			for (int i=0; i< v.size(); i++){
				...
				/*}
								...
								MesquiteMessage.warnProgrammer("exception in broadcastFocusInCell to " + obj + "   " + e);
  				      e.printStackTrace();
				}*/
	...
	void adjustMessage(){
		...
		for (int ic = 0; ic< data.getNumChars(); ic++){
			if (!StringUtil.blank(data.getAnnotation(ic)))
				countChar++;
			for (int it = 0; it< data.getNumTaxa(); it++){
				if (!StringUtil.blank(data.getAnnotation(ic, it)))
					countCell++;
			}
		}
		for (int it = 0; it< data.getNumTaxa(); it++)
			if (!StringUtil.blank(data.getTaxa().getAnnotation(it)))
				countTaxa++;
		 */
	}
