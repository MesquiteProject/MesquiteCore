 ...
 */
package mesquite.io.xExportForStarBeast;
/*~~  */
...
public class xExportForStarBeast extends FileInterpreterI {
...
*/
	...
	private boolean exportAFile(String path, MesquiteString dir, MesquiteString fn, CategoricalData data) {
		if (path != null) {
			MesquiteFile f;
	...
	public boolean exportFile(MesquiteFile file, String arguments) { //if file is null, consider whole project open to export
		...
		if (MesquiteThread.isScripting() || scripting){
			...
			return exportAFile(path, dir, fn, data);
			...
			Taxa taxa = 	 getProject().chooseTaxa(getModuleWindow(), "Choose taxa of the data to be exported"); 
			if (taxa==null) 
				return false;
			...
			for (int iM = 0; iM < numMatrices; iM++){
				data = (CategoricalData)getProject().getCharacterMatrixVisible(taxa, iM, MolecularState.class);
				if (data != null) {		
					if (firstMatrix) {
						path = getPathForExport(arguments, suggested+".nex", dir, fn);
						directoryPath = dir.getValue();
