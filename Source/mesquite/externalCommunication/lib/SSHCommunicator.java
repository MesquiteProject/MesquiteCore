...
public abstract class SSHCommunicator implements XMLPreferencesProcessor {
	...
	protected boolean checkUsernamePassword(boolean tellUserAboutSystem){
		if (StringUtil.blank(getUserName()) || StringUtil.blank(password)){
			...
			String help = "You need an account on the "+getSystemName()+" system to use this service.";
		...
		if (!success && tellUserAboutSystem) {
			MesquiteMessage.discreetNotifyUser("Use of the "+getSystemName()+" service requires an account with the service.");
		}
		return success;
