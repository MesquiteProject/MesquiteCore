...
public class Bits implements Listable, Nameable {
	...
	// used for Associables that might need to record whether reference is to part
	// or in between part
	...
	public void addParts(int starting, int num) {
		...
		/*
		 * //first, put integers up to start of insertion for (int i=0; i<=startingInt;
		 * i++) newArray[i]=array[i]; for (int i=startingInt+1; i< newNumInts; i++)
		 * newArray[i]=0;
		 * 
		 * //next, go bitwise from there for (int ibit=starting; ibit<numBits; ibit++) {
		 * if (isBitOn(ibit)) setBit(newArray, ibit+num); }
		 */
		numBits = newNumBits;
		numInts = newNumInts;
		array = newArray;
	...
	/*
	 * ...........................................................* public void
	 * deletePartsBy Blocks(int[][] blocks) { if (blocks == null || blocks.length ==
	 * 0) return; int availableSlot = blocks[0][0]; //First shift storage toward the
	 * start of the array. Later, we'll delete the leftovers at the end. for (int
	 * block = 0; block<blocks.length; block++) { int startOfPreserved =
	 * blocks[block][1]+1; int endOfPreserved = getSize()-1; if
	 * (block+1<blocks.length) //there's another block coming afterward
	 * endOfPreserved = blocks[block+1][0]-1; for (int ic=startOfPreserved;
	 * ic<=endOfPreserved; ic++) { setBit(availableSlot, isBitOn(ic));
	 * availableSlot++; } } //Next, trim leftovers int newNum = availableSlot;
	 * resetSize(newNum); }
	 * /*...........................................................* public static
	 * boolean[][] deleteColumnsBy Blocks(boolean[][] d, int[][] blocks){ if (d ==
	 * null) return null; if (d.length <= 0) return d; if (blocks == null ||
	 * blocks.length == 0) return d;
	 * 
	 * int numRows= d[0].length; int availableSlot = blocks[0][0];
	 * 
	 * //First shift storage toward the start of the array. Later, we'll delete the
	 * leftovers at the end. for (int block = 0; block<blocks.length; block++) { int
	 * startOfPreserved = blocks[block][1]+1; int endOfPreserved = d.length-1; if
	 * (block+1<blocks.length) //there's another block coming afterward
	 * endOfPreserved = blocks[block+1][0]-1; for (int ic=startOfPreserved;
	 * ic<=endOfPreserved; ic++) { for (int it=0; it<numRows && it< d[ic].length;
	 * it++) { d[availableSlot][it] = d[ic][it]; } availableSlot++; } } //Next, trim
	 * leftovers int newNumColumns = availableSlot; boolean[][] newMatrix=new
	 * boolean[newNumColumns][numRows]; for (int ic=0; ic<newNumColumns; ic++) for
	 * (int it=0; it<numRows && it< d[ic].length; it++) newMatrix[ic][it] =
	 * d[ic][it]; return newMatrix; }
	 * /*...........................................................
	 */
	...
	public void swapValues(int i, int j) {
		if ((inWhichInteger(i) < 0 || inWhichInteger(i) >= array.length)
				|| (inWhichInteger(j) < 0 || inWhichInteger(j) >= array.length))
			return;
	 ...
	 * at least one position with bits1=off and bits2=on - at least one position
	 * with bits1=off and bits2=off
	 */
	...
	public int nextBit(int startBit, boolean on) {
		if (startBit >= getSize())
			return -1;
		// first, see if there are any bits remaining in the current Int that are the
		// same value as "on";
		if (startBit < 0)
			startBit = 0;
	...
	/**
	 * given a bit whichBit, this method returns the first bit ≤ whichBit that has
	 * the same value as whichBit
	 */
	...
	/**
	 * Returns true iff exactly one bit is on. If one bit is on, it returns in
	 * "single" the single bit that is on.
	 */
	...
	/**
	 * returns a string listing the bits on. In the format of NEXUS character, taxa
	 * lists (e.g., "1- 3 6 201-455". The offset is what the first element is to be
	 * numbered (e.g., 0 or 1)
	 */
	 ...
	 */
	/**
	 * converts passed int to string, as its bits representation. Used for
	 ...
	 */
	/**
	 * converts passed long to string, as its bits representation. Used for
	 * development/debugging. �
	 */
