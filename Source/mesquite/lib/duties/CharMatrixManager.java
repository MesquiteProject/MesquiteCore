		...
		if (data.interleaved) {
			...
			while (!done && !(isEndLine(taxonName=parser.getNextToken()))) {
				...
				while (currentCharacter[whichTaxon]<numChars && response!=mesquite.lib.characters.CharacterData.EOL) {
					...
					response = data.setStateQuickNexusReading(currentCharacter[whichTaxon], whichTaxon, parser);
					if (response == CharacterData.ERROR){
						problem = " taxon " + (whichTaxon +1) + ", character " + (currentCharacter[whichTaxon] + 1);
