...
public abstract class DataSearcher extends MesquiteModule  {
	...
	public abstract boolean searchData(mesquite.lib.characters.CharacterData data, MesquiteTable table, boolean useSelectedRowsOnly);
	...
	/** Called to search data in a table or data. This is used if the searching procedure can be done on the selected region in one taxon
	...
	public boolean searchSelectedTaxa(mesquite.lib.characters.CharacterData data, MesquiteTable table, boolean useSelectedRowsOnly){
		ObjectContainer objContainer = new ObjectContainer();
		boolean did=false;
		if (useSelectedRowsOnly && data!=null) {
			if (canSearchMoreThanOnePiece() || data.getNumTaxa()==1)
				return false;
			if (!data.getTaxa().anySelected()) {
				if (!warned)
					discreetAlert( "Sorry, to use the search you need to have at least one taxon selected.");
				warned=true;
				return false;
			}
			for (int j=0; j<data.getNumTaxa(); j++) {
				if (data.getTaxa().getSelected(j))
		...
		else if (table==null && data!=null){    // alter entire matrix
			if (canSearchMoreThanOnePiece() || data.getNumTaxa()==1)
				return false;
			for (int j=0; j<data.getNumTaxa(); j++) {
				if (searchOneTaxon(data,j,0,data.getNumChars(), objContainer)) {
