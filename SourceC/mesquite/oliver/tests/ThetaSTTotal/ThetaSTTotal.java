package mesquite.oliver.tests.ThetaSTTotal;
...
/**This class calculates the statistic ThetaST for a sample of containing taxa.*/
public class ThetaSTTotal extends NumberForMatrix{
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------*/	
	public double meanTheta(MCharactersDistribution data) {
		...
		if(containingTaxa != null && containedTaxa !=null && association != null) { // tests to make sure that the two taxa blocks exist, and proceeds with calcualtions if so
			...
			double[] thetaX = new double[containingTaxa.getNumTaxa()]; // This array holds the values of nucleotide diversity of each containing taxon.
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------*/	
	public double overallTheta(MCharactersDistribution data) {
		...
		nContaining++;
		double theta = ((double)k / ((double)((nContaining * (nContaining - 1)))/2));
...
/**The calculateNumber method checks for a character matrix and association, then, if both are present, calculates ThetaST.*/	
	public void calculateNumber(MCharactersDistribution data, MesquiteNumber result, MesquiteString resultString, CommandRecord commandRec) {
		...
		if(data != null){
			...
			double aveTheta = meanTheta(data);
			double totalTheta = overallTheta(data); 
			if (aveTheta<0 || totalTheta == 0){
				result.setValue(-1);
				logln("There was an error calculating the Total ThetaST.");
			}
			else result.setValue((totalTheta - aveTheta)/totalTheta);
			logln("The average theta = " + aveTheta);
			logln("The overall theta is " + totalTheta);
		
		}
		// else result.setValue(-1); // TODO Throw some sort of alert to log about requiring a data matrix
		if (resultString != null)
			resultString.setValue("ThetaST = " + result.toString());
...
/*--------------------------------------------------------------------------------------------------------------------------------------------------*/	
	public String getExplanation(){
		return "Calculates the statistic ThetaP = (ThetaT - ThetaX)/ThetaT, where ThetaT is the estimate of Theta for the entire matrix, and ThetaX is the average within population/species/association Theta.  Theta is estimated from pairwise differences between taxa.";
	}
